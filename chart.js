...
async function getData(siteID) {
    let data = await fetch('http://localhost:3002/population');
    let parsedData = await data.json();
    allDist = [...parsedData.data];
    allDist.map((item, index, self) => {
...
document.getElementById('dist').addEventListener('change', () => {
    ...
    selectDistData = allDist.filter(e => e.site_id === selectDist);
